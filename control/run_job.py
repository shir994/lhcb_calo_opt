...
def create_job(command, host_dir):
    container = client.containers.run(
        #image=str(container_id),
        image='shir994/lhcb_calo:latest',
        detach=True,
        auto_remove=False,
        command=command,
        volumes={host_dir: {'bind': "/calo/outputs", 'mode': 'rw'}}
        # volumes = {
...
def run_simulation(parameters, job_uuid, n_events):
    ...
    # print("image created")
    container = create_job(command=command, host_dir=os.path.join(config.HOST_DIRECTORY, input_dir))
    print("job started")
    start_time = time()
    result = {
        'uuid': job_uuid,
    ...
    try:
        ...
        optimise_input = json.loads(extract_file_from_container(container, host_dir,
                                                                "optimise_input",
                                                                "/calo/outputs/optimise_input.json"))
